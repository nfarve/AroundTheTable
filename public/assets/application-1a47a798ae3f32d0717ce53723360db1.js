...
(function() {
  ...
  this.textFieldAdjust = function(el) {
    var length, rows;
    ...
    if (length > 60) {
      if ((length % 60) === 0) {
        rows = $(el).attr('rows');
        rows++;
        return $(el).attr('rows', rows);
      }
    }
  };

  this.folder_lookup = function(which) {
    switch (which.value) {
      case "Main":
  ...
  this.addIngredient = function() {
    return $("#tester").append($("#category2").html());
  };
  ...
  this.ready = function() {
    ...
    if ($('#recipe_' + String(count - 1)).length === 0) {
      return $("#prev").attr("disabled", true);
    }
  };
